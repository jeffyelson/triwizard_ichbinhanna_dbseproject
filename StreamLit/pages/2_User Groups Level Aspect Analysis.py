...
def generate_word_cloud_for_category(df, category, wordcloud_category, _wordcloud):
    ...
    for i in range(len(axes)):
        ...
        color_dict = get_color_dict_wc(
            temp["Color"].to_list(), temp[wordcloud_category].to_list()
        )
        count_dict = get_count_dict(
            temp[wordcloud_category].to_list(), temp["Count"].to_list()
        )
    ...
    st.pyplot(fig)
    # plt.savefig("saved_images/wordcloud.png", bbox_inches="tight", transparent=True)
...
def create_table(df_absa, level, category_list):
    ...
    if len(category_list) >= 1:
        for i in range(len(category_list)):
            if i == 0:
                df_category_aspect = (
                    df_absa[df_absa["Category"] == category_list[i]][
                        [level, "Sentiment"]
                    ]
                ...
                df_category_aspect.rename(
                    columns={
                        level: category_list[i] + "_" + level,
                        "Sentiment": category_list[i] + "_Sentiment",
                    },
                    inplace=True,
                )
                aspect_list += df_category_aspect[
                    category_list[i] + "_" + level
                ].to_list()
            else:
                temp = (
                    df_absa[df_absa["Category"] == category_list[i]][
        ...
        styled_df2 = df_ranks.style.apply(
            lambda x: get_aspect_color(x, color_dict), axis=1
        )
...
def generate_bar_graphs(df, category):
    ...
    # plt.savefig("saved_images/cooccuring.png", bbox_inches="tight", transparent=True)
    st.pyplot(fig)
