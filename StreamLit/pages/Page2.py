...
# Plotting Graph
df_selectiona = df_selection["user_category"].value_counts().rename_axis('user_categories').reset_index(name='counts')
...
df_selection1b = df_selection1["user_category"].value_counts().rename_axis('user_categories').reset_index(name='counts')
category_labelsb = df_selection1b.user_categories
category_valuesb = df_selection1b.counts

fig1 = px.pie(df_selection1b,
...
df_selection2c = df_selection2["user_category"].value_counts().rename_axis('user_categories').reset_index(name='counts')
category_labelsc = df_selection2c.user_categories
category_valuesc = df_selection2c.counts

fig2 = px.pie(df_selection2c,
...
datafra_selectiona = dataframe_selection["user_category"].value_counts().rename_axis('user_categories').reset_index(
    name='counts')
...
final1 = dataframe["location_country"].value_counts().rename_axis('Countries').reset_index(name='counts')

final0 = final1[(final1["Countries"] != 'Germany') & (final1["Countries"] != 'Unknown')]['counts'].sum()

Others = [['Others', final0]]
otherdf = pd.DataFrame(Others, columns=['Countries', 'counts'])
final1 = final1.append(otherdf)

n1 = final1[(final1["Countries"] == 'Germany') | (final1["Countries"] == 'Unknown') | (final1['Countries'] == 'Others')]
